...
//  std::cout << numneigh_max << "\n";
  double r[numneigh_max];
  bool keep_neigh[numneigh_max];
...
//    std::cout << ii << "\n";
    for (jj = 0; jj < jnum; jj++) {
      j           =  jlist[jj];
			//following tested in the next loop
...
//			  The keep_neigh boolean and the r array avoid:
							 //	recomputing the distance and
 							//  retesting the types and the distance in the 2nd jj loop

			    keep_neigh[jj] = true;
          r[jj]    =  sqrt(rsq);
  				factor_coord =  r[jj] / rnh[itype][jtype];
        	coord_nr     =  1 - pow(factor_coord, NN[itype][jtype]);
        	coord_dr     =  1 - pow(factor_coord, 2*NN[itype][jtype]);
  
        	coord_tmp[ii][jtype] +=  (coord_nr / coord_dr);
...
// checking CNs
//      for (jtype = 1; jtype <= atom->ntypes; jtype++) {
//			  if (coord_tmp[ii][jtype] > 0) {
//     			printf("i=%i; itype=%i; jtype=%i; coord_tmp=%f; rnh=%f\n", i, itype, jtype, coord_tmp[ii][jtype], rnh[itype][jtype]);
//				}
//      }
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      j           &= NEIGHMASK;
          ...
          jtype =  type[j];
//        std::cout << "r0=" << sqrt(rsq) << " r=" << r  << " cut=" << cut[itype][jtype] << " cutsq=" << cutsq[itype][jtype]  << "\n" ;
          rexp         =  (r[jj]-rmh[itype][jtype])/sigmah[itype][jtype];

					double scale_factor;
          if (coord_tmp[ii][jtype] <= coord[itype][jtype]) {
            scale_factor  =  (coord_tmp[ii][jtype] / coord[itype][jtype]) * hgauss[itype][jtype];
          }
          else {
            double pre_exponent  =  (coord_tmp[ii][jtype] - coord[itype][jtype]);
            scale_factor  =  hgauss[itype][jtype] * exp(-1 * pre_exponent * pre_exponent);
          }
          ugauss               =  (scale_factor / sqrt(MY_2PI) / sigmah[itype][jtype]) * exp(-1 * rexp * rexp);
//            printf("itype=%i; jtype=%i; coord_tmp=%f; rnh=%f; scale_factor=%f; ugauss=%f\n", itype, jtype, coord_tmp[ii][jtype], rnh[itype][jtype], scale_factor, ugauss);
            //std::cout << ii << "\t" << itype << "\t" << jj << "\t" << jtype << "\t" << scale_factor << "\t" << coord_tmp[ii][jtype] << "\t" << coord[itype][jtype] << "\n";

          fpair        =  factor_lj*rexp/r[jj]*ugauss/sigmah[itype][jtype];
...
void PairCoordGaussCut::coeff(int narg, char **arg)
{
  ...
  double NN_one       =   utils::numeric(FLERR,arg[7],false,lmp);

  if (sigmah_one <= 0.0)
    error->all(FLERR,"Incorrect args for pair coefficients");

  double cut_one     = cut_global;
  if (narg == 9) cut_one = utils::numeric(FLERR,arg[7],false,lmp);
